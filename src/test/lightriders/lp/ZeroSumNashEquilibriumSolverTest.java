...
class ZeroSumNashEquilibriumSolverTest {
	...
	void testMatchingPennies() {
		...
		assertArrayEquals(new double[] { 0.5, 0.5 }, solver.findNashEquilibriumStrategy(matrix));
	}
	...
	void testBaseCase() {
		...
		assertArrayEquals(new double[] { 1 }, solver.findNashEquilibriumStrategy(matrix));
	}
